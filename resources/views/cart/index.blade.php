...
@section('content')
    ...
    <section>
        <!-- Section heading -->
        <h1 class="mb-8 text-lg lg:text-[42px] font-black">Корзина</h1>
        ...
        @else
                                ...
                                <td scope="row" class="py-4 px-4 md:px-6 rounded-l-2xl bg-card">
                                    <div class="flex flex-col lg:flex-row min-w-[200px] gap-2 lg:gap-6">
                                        <div class="shrink-0 overflow-hidden w-[64px] lg:w-[84px] h-[64px] lg:h-[84px] rounded-2xl">
                                            <img src="{{ $item->product->makeThumbnail('354x354') }}" class="object-cover w-full h-full" alt="{{ $item->product->title }}">
                                        </div>
                                        <div class="py-3">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-8">
                ...
                <form method="POST" action="{{ route('cart.truncate') }}" class="pb-3 lg:pb-0">
                    @csrf
                    @method('DELETE')
